...
- Zone selection for one-time mowing is experimental. Firmware 3.46.0+47 ignored it and mowed the usual area. Since 2.6.0 the selection is sent the way the Worx app writes it, paired with the flag that separates a deliberate ordered selection from "mow the whole lawn", which earlier releases left out. That is a hypothesis drawn from what the app writes to the weekly schedule, not a confirmed fix, so treat a zone run as something to watch rather than to rely on. The list is ordered: the first id is mowed first.
