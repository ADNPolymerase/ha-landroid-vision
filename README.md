...
- Zone mowing on RTK mowers: mow the zones you pick, in the order you pick, right now. The job runs as a temporary weekly slot, and the schedule is put back once the mower is home.
...
- Zone selection for one-time mowing is ignored by Vision firmware 3.46.0+47: the mower mows its usual area, even with a cut block identical to the one the Worx app writes. Zones set on a weekly schedule slot are honoured, order included, which is what the `worx_vision_cloud.start_zone_mowing` action and the Start zone mowing button use: they add a temporary slot, starting two minutes from now or at a time you choose within the next six days, and remove it once the mower is back on its base. The schedule is saved in Home Assistant first, and put back even if the mower never comes home or Home Assistant restarts meanwhile. A mower resting on its base acknowledges nothing for hours, so a slot it does not answer for is recorded as sent and confirmed against the next schedule it publishes. The action refuses when the Worx automatic schedule is on, when party mode suspends the schedule, or when the job would overlap a slot the mower already has.
