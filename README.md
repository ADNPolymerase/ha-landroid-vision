...
- Zone selection for one-time mowing is ignored by current Vision firmware (verified on 3.46.0+47): the command is accepted and the mower mows its usual area. Zones attached to a weekly schedule slot in the Worx app are honoured by that same firmware, so set them there and leave the one-time selection empty. How the app runs a one-time job in a chosen zone is unresolved.
