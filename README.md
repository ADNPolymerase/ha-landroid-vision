...
- Zone selection for one-time mowing is ignored by current Vision firmware (verified on 3.46.0+47): the command is accepted and the mower mows its usual area. The Worx app can also attach zones to a weekly schedule slot, which a diagnostics dump now shows, but whether the firmware restricts mowing to them has not been verified here. How the app runs a one-time job in a chosen zone is unresolved.
