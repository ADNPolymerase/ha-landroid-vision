...
- Added Computed dock time and Computed error time sensors (diagnostic). Worx exposes lifetime home/charging/error counters but leaves them at 0 on some accounts and models — confirmed on both a WR143E and a Vision Cloud400 whose work-time counter updates normally — so these are measured locally instead and answer a different question: how long the mower has been in its current state. Each resets to 0 the moment the mower leaves that state, and both survive a Home Assistant restart: the start timestamps are persisted, so a mower that has been docked for three hours still reads three hours after a restart rather than starting over. A restored timestamp is dropped as soon as the first refresh shows the mower is no longer in that state; the known trade-off is that a full leave-and-return cycle happening while Home Assistant is down cannot be detected. Rain delay does not count as an error, since it is a normal waiting state rather than a fault.
- The one-time mowing zone picker now uses the zone names configured in the Worx app ("Jardin devant + Jardin Derriere") instead of generic "Zone 1" / "Zones 1, 2" labels. The schedule config holds the zone ids and the map holds the names, so the two are paired on their cutting direction when every direction is distinct, and by map order otherwise; zones keep their numeric label when no name can be resolved, and a name reused across zones gets its id appended. Note: automations calling select.select_option with a literal "Zone 1" need updating — the stored selection itself is by zone id and is unaffected.
- Fixed the Lawn area sensor reporting only the first zone of a multi-zone RTK map, which skewed every progress percentage derived from it. Map zones that are mowed are now summed, while transit zones — corridors the mower only drives through — are excluded. Confirmed on a real three-zone map: 305.45 m² (first zone alone) became 339.15 m², matching the 339.1 m² the Worx app reports, and the day's progress went from a capped 100% to a meaningful 99.7%. Mowed zones are identified by their cutting metadata (cut_type/cut_direction, mirroring the per-zone cutting config in cfg.rtk.zs); maps without any cutting metadata keep the previous first-zone behavior. The cloud's own lawn_size value still wins when the account provides one.
- The map camera now exposes explicit `current_zone_name`, `current_zone_area_m2`, `first_zone_name`, `first_zone_area_m2`, `first_zone_perimeter_m`, `zone_count` and `mowing_zone_count` attributes. The previous `zone_name` / `zone_area_m2` / `zone_perimeter_m` described the map's first zone despite their names suggesting the current one; they are kept as-is so existing dashboards and templates keep working.
