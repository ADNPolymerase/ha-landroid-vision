...
- **No readiness chip repeating the state.** While mowing, the card said "mowing" twice, under the title and in the chip. The chip now only shows when it adds something (ready, battery low, locked...), not when it reads like the state already shown.
...
- **Each RTK zone's mowing pattern and angle.** Two sensors per mowing zone, named after it as in the Worx app: the pattern (natural, parallel, diamond or checker) and the angle in degrees. They are named "Mowing pattern <zone>" and "Mowing pattern <zone> angle", so each zone's pattern and angle sit side by side in the device's sensor list. They are read from the mower's own per-zone config, which it updates once a change made in the app is activated, so they show what the mower will actually mow, zone by zone, whether the zones share the same settings or not. A pattern code not named yet reads `other`, with the code in the `code` attribute. Display only: the settings are still changed in the Worx app.
